...
const MaintainersRegistration = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/maintainer/dashboard'); // Redirects to their dashboard after login
  ...
  return (
    <div className="flex items-center justify-center min-h-screen py-12 bg-gray-50">
      <div className="w-full max-w-lg p-8 bg-white border border-gray-100 shadow-xl rounded-2xl">
        <h2 className="text-3xl font-extrabold text-center text-gray-900 mb-6">Maintainer Registration</h2>
        {error && <div className="p-3 mb-4 text-sm text-red-700 bg-red-100 rounded-lg">{error}</div>}
        <form onSubmit={handleSubmit} className="space-y-4">
          <input type="text" name="name" required onChange={handleChange} className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Full Name" />
          ...
          <button type="submit" className="w-full py-3 font-bold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700">Register as Maintainer</button>
        </form>
