...
const ManageKitchenMeals = () => {
  ...
  const handleExportOrders = async () => {
    try {
      ...
      const kitchenLines = [lines[0]];
      for (let i = 1; i < lines.length; i++) {
        if (lines[i].includes('"Kitchen"')) kitchenLines.push(lines[i]);
      }
      const blob = new Blob([kitchenLines.join('\n')], { type: 'text/csv;charset=utf-8;' });
  ...
  const displayedOrders = orders.filter((order) => {
    ...
    return studentName.includes(term) || slotLabel.includes(term) || status.includes(term);
  });
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-8">
          <div className="mb-6">
            ...
            <h2 className="text-2xl font-semibold mt-4 text-gray-800">Kitchen Slot Management</h2>
            <p className="text-gray-500 text-sm mt-1">Slots, capacities, and student kitchen bookings.</p>
          </div>

          <div className="mt-2">
            <h3 className="text-xl font-semibold mb-4 text-gray-800">Kitchen Slot Capacity</h3>
                ...
                <tbody className="divide-y divide-gray-100 text-sm">
                  {slots.map((slot) => (
                    <tr key={slot._id} className="hover:bg-gray-50">
                      ...
                      <td className="p-4">
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-bold ${slot.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                        >
                      ...
                      <td className="p-4 text-right space-x-2">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg text-xs font-bold bg-amber-50 text-amber-700 hover:bg-amber-100"
                        >
          ...
          <div className="mt-10">
            <h3 className="text-2xl font-semibold mb-6 text-gray-800">Student Kitchen Bookings</h3>
                  ...
                  <tbody className="divide-y divide-gray-100 text-sm">
                    {paginatedOrders.map((order) => (
                      <tr key={order._id} className="hover:bg-gray-50">
                        ...
                        <td className="p-4 text-gray-800 font-semibold">{order.bookingDate}</td>
                        <td className="p-4 text-gray-600">{order.slotLabel}</td>
                        <td className="p-4">
                          <span
...
export default ManageKitchenMeals;
