...
const Login = () => {
  ...
  // Helper component to render a beautiful stat card (Crash-proof)
  const StatCard = ({ title, icon, dataCount }) => (
    <div className="bg-white/10 backdrop-blur-md border border-[#CBDDE9]/20 p-5 rounded-2xl shadow-xl hover:bg-white/20 transition-all duration-300">
      ...
      {dataCount !== undefined && dataCount !== null ? (
        <div className="flex justify-between items-end">
          <div>
            <p className="text-[#CBDDE9] text-xs uppercase tracking-wider font-bold mb-1">Total Active</p>
            <p className="text-3xl font-bold text-white">{dataCount}</p>
          </div>
        </div>
      ) : (
        <div className="animate-pulse flex space-x-4 h-12 items-center text-[#CBDDE9] text-sm font-medium">
          Loading data...
        </div>
      )}
    </div>
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-[#2872A1] p-12 flex-col justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg mx-auto w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <StatCard title="Students" icon="🎓" dataCount={stats?.students} />
            <StatCard title="Laundry" icon="👕" dataCount={stats?.laundryPartners} />
            <StatCard title="Meals" icon="🍲" dataCount={stats?.mealSuppliers} />
            <StatCard title="Maintenance" icon="🔧" dataCount={stats?.maintainers} />
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-md p-8 bg-white border border-gray-100 shadow-2xl shadow-[#CBDDE9]/40 rounded-3xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input 
                ...
                className="w-full p-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#2872A1] focus:border-[#2872A1] outline-none transition-all" 
            ...
            <button 
              type="submit" 
              className="w-full py-4 mt-4 font-bold text-white transition-all bg-[#2872A1] rounded-xl shadow-lg shadow-[#CBDDE9] hover:bg-[#1f5a80] hover:-translate-y-0.5"
            >
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 text-center">
            <p className="text-sm text-gray-600">
              ...
              <Link to="/register" className="font-bold text-[#2872A1] hover:underline ml-1">
